...
{
    // 输出场景状态结果
    if (step == Step::NONE || !params->config.stop)
...
void FsmStop::run(Mat &img)
{
    ...
    case Step::NONE: // AI未识别
    {
        ...
        {
            if (params->results[i].type == LABEL_ICAR && !params->ctrl.parking) // 障碍物：智能车
            {
                overlap = getRoadCarPloy(params->results[i]); // 计算多边形重合度
                if (overlap >= params->config.overlap)        // 重合度判断
                    countRec++;
                else if (params->track->pointsEdgeLeft.size() < ROWSIMAGE / 5 || params->track->pointsEdgeRight.size() < ROWSIMAGE / 5)
                    countRec++;
                else if (params->track->pointsEdgeLeft[0].x < (params->results[i].y + params->results[i].height / 2) ||
                         params->track->pointsEdgeRight[0].x < (params->results[i].y + params->results[i].height / 2))
                    countRec++;
                break;
            }
            else if (params->results[i].type == LABEL_GATE) // 障碍物：道闸
            {
                countRec++;
                break;
            }
    ...
    case Step::ENABLE: // 场景使能
    {
        ...
        for (int i = 0; i < params->results.size(); i++)
        {
            if (params->results[i].type == LABEL_ICAR && !params->ctrl.parking) // 障碍物：智能车
            {
                countSes = 0;
                overlap = getRoadCarPloy(params->results[i]); // 计算多边形重合度
                if ((params->results[i].y + params->results[i].height) > ROWSIMAGE * 0.3 &&
                    overlap >= params->config.overlap) // 停车距离计算
                {
                    countRec++;
                    timeout = 0;
                    break;
            ...
            else if (params->results[i].type == LABEL_GATE) // 障碍物：道闸
            {
                countSes = 0;
                timeout = 0;
                if ((params->results[i].y + params->results[i].height) > ROWSIMAGE * 0.3) // 停车距离计算
                {
                    countRec++;
                    break;
                }
    ...
    case Step::STOP: // 停车
    {
        ...
        for (int i = 0; i < params->results.size(); i++)
        {
            ...
            {
                overlap = getRoadCarPloy(params->results[i]); // 计算多边形重合度
                if (overlap >= params->config.overlap)
                    countSes = 0;
                else if (params->track->pointsEdgeLeft.size() < ROWSIMAGE / 5 || params->track->pointsEdgeRight.size() < ROWSIMAGE / 5)
                    countSes = 0;
                else if (params->track->pointsEdgeLeft[0].x < (params->results[i].y + params->results[i].height / 2) ||
